...
export function renderTerminalDashboard(snapshot, { width = 80 } = {}) {
  ...
  return [
    ...
    !live && Number.isFinite(summary.available_equity_usd) ? kv("Available Equity:", `$${formatNumber(summary.available_equity_usd, 2)}`) : null,
    !live && Number.isFinite(summary.max_drawdown_usd) ? kv("Max Drawdown:", `$${formatNumber(summary.max_drawdown_usd, 2)} (${formatNumber(summary.max_drawdown_pct ?? 0, 1)}%)`) : null,
    !live && Number.isFinite(summary.daily_pnl_usd) ? kv("Daily PnL:", `${summary.daily_pnl_usd > 0 ? "+" : ""}$${formatNumber(summary.daily_pnl_usd, 2)}`) : null,
    "",
    separator(safeWidth),
    "",
    section("Session"),
